...
def score_fold(
    ...
    tiebreak_col, return the ranked y labels. combined_score is ranked with the raw score
    (oof_rf_score, the only name it has in this script — see note below) as the tiebreak,
    to match 02_build_queue.py's real sort. This function should validate what's actually
    deployed, not a simpler version of it.
    """
...
def main() -> None:
    ...
    for fold_num, (_, test_idx) in enumerate(gkf.split(X, y, groups), start=1):
        ...
        combined_ranked = score_fold(model_df, test_idx, "combined_score", y, tiebreak_col="oof_rf_score")
